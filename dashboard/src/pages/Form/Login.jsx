...
const Login = () => {
  ...
  const login = async (values) => {
    try {
      ...
      } else if (err.response.data.msg === 'User not approved. ') {
        setCurrentError('User not approved.');
        console.log(currentError);
      ...
      setCurrentError(err.response.data.msg);
      setIsLoading(false);
      // refresh();
    }
  };
  ...
  return (
    <div className='w-screen h-screen flex flex-col items-center justify-center background-gradient bg-cover'>
      <div className='flex justify-center align-middle'>
        <img
          className='w-20'
          src={logo}
        />
        <h1 className='text-4xl text-gray-200 m-auto font-bold'>
          GOOBULL
        </h1>
      </div>
      <div className='m-8 lg:m-0 p-6 lg:p-16 w-10/12 md:w-7/12 lg:w-5/12 bg-white shadow-2xl rounded-t-3xl rounded-br-3xl'>
        ...
        <form className='flex flex-wrap flex-col justify-center mt-5'>
          ...
          <div className='flex flex-col w-full items-center mt-5 px-2'>
            ...
            {errorss && (
              <div className='mt-2 text-sm flex flex-col text-center'>
            ...
            <a
              ...
              href='/register'
            >
              No Existing Account? <b>Register</b>
            </a>
          </div>
        </form>
      </div>
